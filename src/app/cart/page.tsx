...
import { getCartItems, updateCartItemQuantity, removeCartItem } from '@/lib/firestore';
...
export default function CartPage() {
  ...
  const handleQuantityChange = async (listingId: string, newQuantity: number) => {
    if (!user || newQuantity < 1) return;
    await updateCartItemQuantity(user.uid, listingId, newQuantity);
  };
  
  const handleRemoveItem = async (listingId: string) => {
      if (!user) return;
      await removeCartItem(user.uid, listingId);
  }
